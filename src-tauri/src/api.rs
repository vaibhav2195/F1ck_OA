...
#[derive(Debug, Serialize, Deserialize)]
pub struct f1ck_OAPromptsResponse {
    prompts: Vec<f1ck_OAPrompt>,
    total: i32,
...
// Fetch f1ck_OA Prompts API
#[tauri::command]
pub async fn fetch_prompts() -> Result<f1ck_OAPromptsResponse, String> {
    let app_endpoint = get_app_endpoint()?;
    let api_access_key = get_api_access_key()?;
    ...
    let prompts_response: f1ck_OAPromptsResponse = response
        .json()
        .await
