...
export const Audio = ({
  ...
  const { selectedSttProvider, f1ck_OAApiEnabled, selectedAudioDevices } =
    useApp();
  ...
  return (
    <Popover open={micOpen} onOpenChange={setMicOpen}>
      <PopoverTrigger asChild>
        {(f1ck_OAApiEnabled || speechProviderStatus) && enableVAD ? (
          <AutoSpeechVAD
      ...
      <PopoverContent
        align="end"
        side="bottom"
        className={`w-80 p-3 ${f1ck_OAApiEnabled || speechProviderStatus ? "hidden" : ""
          }`}
        sideOffset={8}
      >
