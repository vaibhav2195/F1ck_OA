...
// f1ck_OA STT function
async function fetchf1ck_OASTT(audio: File | Blob): Promise<string> {
  try {
    ...
    return `f1ck_OA STT Error: ${errorMessage}`;
  }
}
...
export async function fetchSTT(params: STTParams): Promise<string> {
  ...
  try {
    ...
    if (usef1ck_OAAPI) {
      return await fetchf1ck_OASTT(audio);
    }
