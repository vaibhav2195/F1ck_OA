...
export const useCompletion = () => {
  ...
  const submit = useCallback(
    async (speechText?: string) => {
      ...
      try {
        ...
        if (!selectedAIProvider.provider && !usef1ck_OAAPI) {
          setState((prev) => ({
            ...prev,
  ...
  useEffect(() => {
    ...
    const handleStorageChange = async (e: StorageEvent) => {
      if (e.key === "f1ck_OA-conversation-selected" && e.newValue) {
        try {
        ...
        if (!hasPermission) {
          ...
          if (!hasPermissionNow) {
            setState((prev) => ({
              ...prev,
              error:
                "Screen Recording permission required. Please enable it by going to System Settings > Privacy & Security > Screen & System Audio Recording. If you don't see f1ck_OA in the list, click the '+' button to add it. If it's already listed, make sure it's enabled. Then restart the app.",
            }));
