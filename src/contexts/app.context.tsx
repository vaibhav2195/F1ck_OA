...
export const AppProvider = ({ children }: { children: ReactNode }) => {
  ...
  // f1ck_OA API State
  const [f1ck_OAApiEnabled, setf1ck_OAApiEnabledState] = useState<boolean>(
  ...
  const loadData = () => {
    ...
    // Load f1ck_OA API enabled state
    const savedf1ck_OAApiEnabled = safeLocalStorage.getItem(
      STORAGE_KEYS.f1ck_OA_API_ENABLED
    );
    if (savedf1ck_OAApiEnabled !== null) {
      setf1ck_OAApiEnabledState(savedf1ck_OAApiEnabled === "true");
    }
  ...
  useEffect(() => {
    const checkImageSupport = async () => {
      if (f1ck_OAApiEnabled) {
        // For f1ck_OA API, check the selected model's modality
        try {
          const storage = await invoke<{
            selected_f1ck_OA_model?: string;
          }>("secure_storage_get");

          if (storage.selected_f1ck_OA_model) {
            const model = JSON.parse(storage.selected_f1ck_OA_model);
    ...
    checkImageSupport();
  }, [f1ck_OAApiEnabled, selectedAIProvider.provider]);
  ...
  const setf1ck_OAApiEnabled = async (enabled: boolean) => {
    setf1ck_OAApiEnabledState(enabled);
    safeLocalStorage.setItem(STORAGE_KEYS.f1ck_OA_API_ENABLED, String(enabled));
    ...
    if (enabled) {
      try {
        ...
        console.debug("Failed to check f1ck_OA model image support:", error);
        setSupportsImages(false);
      }
    } else {
